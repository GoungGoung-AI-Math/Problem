...
TOKEN_PATH="/home/ec2-user/project-root/requirements/vault/certs/springProblem/vault_token.txt" # Updated token path
...
ssh -t -i $PEM_PATH $EC2_USER@$EC2_IP << EOF
  ...
  DB_USERNAME=\$(echo \$SECRETS | jq -r '.data.data.username_problem_db')
  DB_PASSWORD=\$(echo \$SECRETS | jq -r '.data.data.password_problem_db')
